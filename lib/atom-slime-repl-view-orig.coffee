...
class REPLView extends View
  ...
  initialize: (@swank) ->
    ...
    # Set the grammar, once it's loaded
    desired_grammar = 'source.python'
    grammar = atom.grammars.grammarForScopeName(desired_grammar)
    if grammar
      console.log "Setting grammar"
      @ed.setGrammar grammar
    else
      # The grammar may not have been loaded & added to registry yet,
      # so wait for it.
      @subs.add atom.grammars.onDidAddGrammar (g) =>
        if g.scopeName == desired_grammar
          @ed.setGrammar g
